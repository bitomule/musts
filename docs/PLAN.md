...
6. **`bazel_picks_deepest_target`** — root + child build checks both apply; the reference `bazel/build` emits one task for the deeper target whose `satisfies` array contains **both** check_ids (`App/Login/login-build` *and* `root/app-build`), and lists `root/app-build` in `ignored_checks` as diagnostic info. One evidence submission produces one ledger row per check, so the next `validate` is clean — both checks are green for their respective scope_hashes. A subsequent edit **inside** `App/Login/` re-opens only the child via the §4.5 carve-out; an edit **outside** `App/Login/` (e.g. top-level `README.md`) re-opens only the root scope and `bazel/build` emits a root-only task with no subsumption. Both directions validate the carve-out and the subsumption.
...
- **Convergence model (subsumption via resolve-time `satisfies`)**: a check goes green only when an extension lists it in an evidence-accept's `satisfies` array. The agent's loop terminates because `validate` reports "no pending tasks". An extension may **subsume** dirty ancestor checks by including their check_ids in a deeper task's resolve-time `satisfies` list — the core then writes one ledger row per check on accept, so the next `validate` finds the ancestor green for its scope_hash. This is the path the reference `bazel/build` extension takes: when both root and child build checks are dirty in the same run, the child task's `satisfies` includes both check_ids, and one evidence submission closes the loop on both. Ancestors are also listed in `ignored_checks` for diagnostic visibility. Extensions that prefer not to subsume can still return checks in `ignored_checks` alone; they'll be re-sent to the extension on the next `validate` (cheap) and the loop still terminates because no task is emitted for them — completion is "no pending tasks", not "every check has a ledger row".
